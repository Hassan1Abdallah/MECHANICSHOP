    ...
    public sealed class Result<TValue> : IResult<TValue>
    {
        ...
        public Result(TValue? value, List<Error>? errors, bool isSuccess)
        {
            ...
            else
            {
                ...
                {
                    throw new ArgumentException("Provide at least one error", nameof(value));
                }
                _errors = errors;
                _value = default!;
                IsSuccess = false;
        ...
        private Result(List<Error> errors)
        {
            if(errors is null || errors.Count == 0)
            {
                throw new ArgumentException(
                    "Connot create an ErrorOr<TValue>  from an empty Collection of errors. provide at least one error.",
                    nameof(errors));
            }
            _errors = errors;
            IsSuccess = false;
