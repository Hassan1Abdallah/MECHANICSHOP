...
namespace MechanicShop.Application.Common.Errors
{
    public static class ApplicationsErrors
    {
        public static Error WorkOrderOutsideOperatingHour(DateTimeOffset startAtUtc, DateTimeOffset endAtUtc) =>
            Error.Conflict("ApplicationsErrors.WorkOrder.Outside.Operating.Hour",$"The WorkOrder time ({startAtUtc} ? {endAtUtc}) is outside of store operating hours.");

        public static Error WorkOrderNotFound => Error.NotFound("ApplicationsErrors.WorkOrder.NotFound", "WorkOrder does not exist ");

        public static Error LaborOccupied => Error.Conflict("Employee.LaborOccupied", "Labor is already occupied during the requsted time");
